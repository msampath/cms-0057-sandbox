...
- **Epic on FHIR, Backend Services** — a separate outbound path from the above: the sandbox signs an RS384 `client-confidential-asymmetric` JWT assertion with its own keypair and calls Epic's `client_credentials` token endpoint directly, the path Epic's Developer Testing Guide documents as the intended way for backend apps to connect. Assertion construction, JWKS publication, and Cloud Run's own reachability of that JWKS were all confirmed correct. See `docs/integrations.md` for whether Epic ultimately granted the token exchange — that section carries the real answer rather than a guess made here.

Together these fill in the columns the sandbox was missing before: EHR-side callers on the inbound and a clearinghouse on the outbound, with a conformance oracle grading both, plus a second outbound path to Epic specifically. See [docs/integrations.md](integrations.md) for setup steps per tool.
