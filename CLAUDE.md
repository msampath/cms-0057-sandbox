...
Any README edits, architecture docs, comments written in the user's voice, or other user-facing prose must follow these rules. A longer voice fingerprint used to live at `C:\Users\Admin\OneDrive\Desktop\surakshith-voice-fingerprint.md`, but that file is no longer there, so the rules below are authoritative. Ask the user for the fingerprint file if a judgment call is not covered here.
