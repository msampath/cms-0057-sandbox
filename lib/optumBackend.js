...
// Same MemberPatient + CoverageToMatch shape this sandbox's own
// $member-match endpoint expects (app/api/payer-to-payer/member-match/route.js),
// carrying one of our four demo patients' real demographics instead of a
// fixed empty body. Optum's sandbox holds its own unrelated test members,
// so a demo patient legitimately landing in NonMatchedMembers is the
// honest outcome, not a bug -- this proves our request reached a real
// $bulk-member-match implementation and got a real verdict back.
function buildBulkMemberMatchParameters(patient) {
  return {
    resourceType: 'Parameters',
...
export async function bulkMemberMatch(patient) {
  const mode = optumMode();
  if (mode === 'disabled') {
    const err = new Error('Optum integration disabled (OPTUM_ENABLED=off)');
    err.status = 501;
  ...
  const request = buildBulkMemberMatchParameters(patient);
  if (mode !== 'live') {
